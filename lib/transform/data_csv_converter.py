...
def convert_data_to_csv(data_transformation: DataTransformation, source_path, results_path, clean=False,
                        quiet=False):
        ...
        for input_port in data_transformation.input_ports:
            for file in input_port.files:
                for dataset in file.datasets:
                    source_file_path = os.path.join(source_path, input_port.id, file.target_file_name)
                    target_file_path = os.path.join(results_path, input_port.id, dataset.target_file_name)
                    ...
                    try:
                        dataframe = pd.read_excel(
                            ...
                            usecols=list(range(dataset.skip_cols, dataset.skip_cols + len(dataset.names))),
                            skiprows=dataset.skip_rows,
                        )

                        dataframe = dataframe.astype({name.name: name.type for name in dataset.names}, errors="ignore")

                        if dataset.drop_columns is not None:
                            dataframe = dataframe.drop(columns=dataset.drop_columns, errors="ignore")
